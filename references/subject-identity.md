...
[OUTPUT]: 对外提供证据受限的猫咪身份锚点、身份安全的微表情、多猫处理和参考图冲突规则，不输出性格判断
...
The renderer receives these anchors for all nine cat depictions. Micro-expressions live on individual sticker concepts as acting directions; they are not derived from a personality profile. Change at most two identity-safe cues per sticker: eyelid openness, gaze direction, ear angle, muzzle tension, or slight head tilt.
...
Allow slight head tilt, small eyelid/gaze/ear/muzzle changes, a clean jawline crop, ordinary prop interaction, and one or two partial front paws only when essential for readability. Never expose the neck, shoulders, chest, torso, belly, arms, or legs. Keep at least four identity anchors readable whenever a prop partially occludes the face.
