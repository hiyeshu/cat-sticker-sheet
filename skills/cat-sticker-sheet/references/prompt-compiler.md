<!--
[INPUT]: 依赖 sheet.schema.json 的字段契约、subject-genome.md 的身份与视觉性格，以及 shell-variation.md 的猫头造型、表情和特殊材质分配结果
...
不要在送给渲染器的文本里点名任何品牌、联名、艺术家或它们的具体道具。**告诉渲染器「别画 X」的同时，你已经把 X 放进了它的条件里**；图像模型对否定式的遵从本来就弱，这类句子会反向拉高漂移概率。列举对方的具体造型清单尤其危险。

正确做法：IP 边界只作为规则写在 SKILL.md，供模型在**选择内容时**遵守；编译产物里一个品牌名都不出现。
...
`pieces[].mechanism` 九片九种、双射；至少五种表情、四类材质、两片高荒诞度、六个灵感领域；`accents` 至少四枚声明 `residue_of`。这些由校验器裁决，不靠自觉。
...
- 明确声明未生成、未做视觉验收；
- 不得出现未替换的占位符、`various funny props` 这类含糊表达，或任何品牌名。
